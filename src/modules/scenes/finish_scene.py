...
class FinishScene(Scene):
    def __init__(self, score: int, dark_background: pygame.Surface, light_background: pygame.Surface):
        super().__init__()
        self.score = score
    ...
    def start_scene(self):
        ...
        if str(StateManager.get_state('player', 'grade'))[0] == 'C' or str(StateManager.get_state('player', 'grade'))[0] == 'D' or str(StateManager.get_state('player', 'grade'))[0] == 'F':
            self.description_msg = "조금 더 분발하세요..."
        else:
            self.description_msg = "훌륭해요!"

    def draw(self, screen: pygame.Surface):
        '''
        game_over 화면에 score와 학점을 그린다
        '''
        if self.description_msg == "훌륭해요!":
            blit_item(screen, self.light_background, topleft=(0, 0))
        else:
            blit_item(screen, self.dark_background, topleft=(0, 0))
